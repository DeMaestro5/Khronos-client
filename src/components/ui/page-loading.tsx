// Enhanced PageLoading component with content-specific features
...
interface PageLoadingProps {
  ...
  showGrid?: boolean;
  gridItems?: number;
  // New props for content-specific loading
...
const PageLoading: React.FC<PageLoadingProps> = ({
  ...
  return (
    <div
      className={`${
        isContentDetail ? '' : 'min-h-screen'
      } bg-theme-secondary p-3 sm:p-4 lg:p-6 relative overflow-hidden transition-colors duration-300`}
    >
      ...
      <div className='relative z-10 space-y-6 max-w-7xl mx-auto'>
        ...
        {isContentDetail && contentPreview && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className='bg-theme-card/50 backdrop-blur-sm rounded-2xl p-6 border border-theme-primary mb-8'
          >
              ...
              <div className='flex items-start justify-between'>
                <div className='space-y-2 flex-1'>
                  {contentPreview.title ? (
                    ...
                    <div className='h-6 shimmer-theme rounded w-2/3'></div>
                  )}

                  <div className='flex gap-2 items-center'>
                    {contentPreview.type && (
                      <span className='text-xs px-2 py-1 bg-theme-secondary/50 text-theme-secondary rounded-full'>
                        {contentPreview.type}
                      </span>
                    )}
                ...
                <div className='w-12 h-12 shimmer-theme rounded-full'></div>
              </div>

              {contentPreview.tags && contentPreview.tags.length > 0 && (
                <div className='flex gap-2 flex-wrap'>
                  {contentPreview.tags.map((tag, index) => (
                    <span
                      key={index}
                      className='text-xs px-2 py-1 bg-theme-primary/10 text-theme-secondary rounded-full'
                    >
                      {tag}
                    </span>
                  ))}
        ...
        {/* Header Section - Only show if not content detail or no preview */}
        {(!isContentDetail || !contentPreview) && (
        ...
        {/* Controls Section - Only show if not content detail */}
        {!isContentDetail && (
